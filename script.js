    ...
    function createPopup() {
      ...
      if (typeof data[academyName] === 'object' && !Array.isArray(data[academyName])) {
        ...
        subGroups.forEach(group => {
          ...
          listItem.addEventListener('click', () => {
            contentDiv.innerHTML = `
              ...
              <h2>${group}</h2>
              <ul style='list-style: none; text-align: center;' id="${group}">
                ${data[academyName][group].map(character => {
                  return `<li>
                            <img src="${character.image}" alt="${character.name}" style="width: 100px; height: 100px; cursor: pointer;" />
                            <p>${character.name}</p>
                          </li>`;
                }).join('')}
              </ul>
        ...
        students.forEach(student => {
          const listItem = document.createElement('li');
          listItem.innerHTML = `
            <img src="${student.image}" alt="${student.name}" style="width: 100px; height: 100px; cursor: pointer;" />
            <p>${student.name}</p>
          `;
...
li img {
  width: 100px;
  height: 100px;
  object-fit: cover; /* 이미지 크기에 맞춰 잘라냄 */
  cursor: pointer;
}
